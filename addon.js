...
const manifest = {
    ...
    description: "Hollywood & Western animation movies + Live sports & TV streaming via RebelIPTV.",
    ...
    catalogs: [
        ...
        // ── Live Sports & TV ──────────────────────────────────────────────────
        { type: "tv", id: "iptv_events",   name: "🏆 Live Sports (Active Now)" },
    ...
    if (type === "tv") {
        if (id === "iptv_events") {
            ...
            if (Array.isArray(data)) {
                // Only show events that have an active stream, to prevent empty 404s in Stremio
                const activeEvents = data.filter(e => e.hasStream === true || e.streamOnline === true);
        ...
        if (id === "iptv_channels") {
            ...
            if (Array.isArray(data)) {
                // Filter out hidden/offline channels
